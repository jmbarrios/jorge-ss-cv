...
def procesar_imagen(ruta_imagen: Path, carpeta_salida: Path, margen: int = 5):
    ...
    detector = pw_detection.MegaDetectorV6(version="MDV6-yolov10-c") #Aquí esta un modelo seleccionado
    ...
    for i, result in enumerate(resultados):
        ...
        for j, caja in enumerate(cajas):
            ...
            # Guardar recorte en la carpeta de salida que se creara en la misma ruta
...
#Esta función hará que en ves de recibir como arg una imagen
#Pueda hacerse con una carpeta 
def procesar_carpeta(carpeta: Path, margen: int = 5):
    carpeta_salida = carpeta / "recortes"
...
#Entonces esta función recibe una carpeta y un tamaño de margen (path)
